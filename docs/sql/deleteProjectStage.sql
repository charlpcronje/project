CREATE DEFINER=`administrator`@`localhost` PROCEDURE `deleteProjectSd`(IN `pProjectSdId` BIGINT)
    NO SQL
BEGIN
    ...
    if (vCount > 0 ) then 
		signal sqlstate '45000'
		set message_text = 'Can not delete this record.  It has linked Project Stage(s).';
	end if;	
