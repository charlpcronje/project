...
//editProjectExpense -- Start
function editProjectExpense(rowSelector, participantIdPayer, participantPayerSystemName) {
	...
	if (rowSelector != null) {
		disNuweEen = false;
		data = pexParticipantExpenseTable.row(rowSelector).data(); //Will give us the data of the double-clicked row 
		$("#peProjectId").val(data.projectId);
		$("#peProjectName").val(data.projectName);
		$("#peProjectExpenseId").val(data.projectExpenseId);
		// Get the ProjectExpense record
		var queryUrl = "/rest/ignite/v1/project-expense/view/" + data.projectExpenseId;
		...
		$.ajax({
			...
			success: function(data) {
				populateExpenseFields(data);
				initializeProjectExpenseFiles();
			},
...
}  //  editProjectExpense -- END




function populateExpenseFields(data) {
	pexParticipantExpenseTable.rows().deselect();
	...
	populateSelect("peExpenseTypeId",
		       "/rest/ignite/v1/expense-type/not-allowance",
		       "expenseTypeId",
		       "name",
		       data.expenseTypeId,
		       true,
	...
	setElementEnabled("savePurchaseButton", false);
	somethingChangedInDialog = false;
	askToSaveDialog = false;
	showModalDialog("projectExpenseDialog");
...
function savePurchase() {
	...
	saveEntry(postUrl, postData, null, "The Project Expense has been saved.", pexParticipantExpenseTable, function(request, response) {
...
function editSelectParticipantMadePurchase223() {
	selectParticipantIndividual223("peParticipantIdMadePurchase", "peParticipantMadePurchaseSystemName");
	peDialogChanged();
}


function editSelectParticipantVendor223() {
	selectParticipantVendor223("peParticipantIdVendor", "peParticipantVendorSystemName");
	peDialogChanged();
}
...
function peDialogChanged() {
	askToSaveDialog = true;
	somethingChangedInDialog = true;
	setElementEnabled("savePurchaseButton", true);
}
//peDialogChanged -- End
...
function selectParticipantVendor223(targetId, targetName) {   
	
	var queryUrl="/rest/ignite/v1/participant/all-in-view";
	
	console.log("In projectExpenseDialog.js  selectParticipantVendor223:  targetId:" + targetId + "  targetName:" + targetName + "   queryUrl:" + queryUrl);
...
function selectParticipantIndividual223(targetId, targetName) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			var columnDefs = [
				...
				,{
					render: function(data, type, row) {
						return "<input type='checkbox' readonly onclick='return false;' checked>";
					},
					className: "dt-center",
					targets: 6
...
}  // selectParticipantIndividual223  --  End
...
function initializeProjectExpenseFiles() {
//	setElementEnabled("promptDeleteFileBtn", false);
	var projectExpenseId = $("#peProjectExpenseId").val();
	
	var queryUrl = "/rest/ignite/v1/project-expense-file/list-project-expense-id/" + projectExpenseId;
	console.log("In initializeProjectExpenseFiles queryUrl:=" + queryUrl);
	...
	var buttonsArray = [
		{
			...
			action: function(e, dt, node, config) {
				...
				pdProjectExpenseFiles.rows().deselect();
				hideDocumentDisplayer();
			}
		},
		{
	...
	hideDocumentDisplayer();
	updateProjectExpenseFileToolbar();
};  //initializeProjectExpenseFiles -- END


function showTable() {
	setDivVisibility("emptyProjectExpenseFilesTablePanel", "none");
	setDivVisibility("projectExpenseFilesTablePanel", "block");
};

function hideTable() {
	setDivVisibility("emptyProjectExpenseFilesTablePanel", "block");
	setDivVisibility("projectExpenseFilesTablePanel", "none");
};
...
function populateExpenseUnitTypeCode223() {
	...
	var selectedExpenseTypeId = $("#peExpenseTypeId").val(); //Tells JQuery to fetch the value of the element id called niwBankId from the html page 
	if ((selectedExpenseTypeId == null) || (selectedExpenseTypeId == "")){
		$("#peUnitTypeCode").val("");
		return
	}
	console.log("selectedExpenseTypeId:=" + selectedExpenseTypeId)
...
function initializeProjectExpense() {
