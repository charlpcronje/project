...
// Definition  - Start
function initializeDefinitionTable() {
	hideTaskTypeTable();
	...
	var buttonsArray = [
		...
		{
			attr: {
				id: "promptDeleteDefinitionBtn"
			},
			titleAttr: "Delete",
			text: "<i class='fas fa-minus'></i>",
			className: "btn btn-sm",
			action: function( e, dt, node, config ) {
				promptDeleteDefinition();
			}
		}
	];

	definitionTable = initializeGenericTable("definitionTable", 
			                            "/rest/ignite/v1/definition",
			                            columnArray,
			                            columnDefsArray,
			                            buttonsArray,
			                            function(rowSelector) {
											editDefinition(rowSelector);
										},
										null,
										25,
	...
	definitionTable.off('deselect');
	definitionTable.on('deselect', function () {
		hideTaskTypeTable();
		updateDefinitionToolbarButtons();
	} );

	definitionTable.off('select');
	definitionTable.on('select', function ( e, dt, node, config) {
...
function updateDefinitionToolbarButtons() {
	var hasSelected = definitionTable.rows('.selected').data().length > 0;

	setTableButtonState(definitionTable, "promptDeleteDefinitionBtn", hasSelected);	
}

function editDefinition(rowSelector) {
	var data = {};
			
	if ((rowSelector != undefined) && (rowSelector != null)) {
		data = definitionTable.row(rowSelector).data();
	};
	
	definitionTable.rows().deselect();
	
	$("#aDlgDefinitionId").val(data.definitionId);
	$("#aDlgName").val(data.name);
	$("#aDlgDescription").val(data.description);

	$("#aDlgDefinitionId").prop("readonly", true);
	
	showModalDialog("definitionDialog");
}


function saveDefinition() {
	var postUrl = "/rest/ignite/v1/definition/new";
	var postData = {
			definitionId: $("#aDlgDefinitionId").val(),
			name: $("#aDlgName").val(),
	...
	if ((postData.definitionId != null) && (postData.definitionId != "")) {
		//This is an update
		postUrl = "/rest/ignite/v1/definition";
	}
	...
	saveEntry(postUrl, postData, "definitionDialog", "The Definition has been saved.", definitionTable);
	// Select the newly saved record 
}

function promptDeleteDefinition() {
	showDialog("Confirm?",
		       "Are you sure that you wish to delete the selected Definition?",
		       DialogConstants.TYPE_CONFIRM, 
		       DialogConstants.ALERTTYPE_INFO, 
		       function (e) {
					deleteDefinition(definitionTable);
			   }
	);
}

function deleteDefinition(tbl) {
	var postUrl = "/rest/ignite/v1/definition/delete";
	var row = tbl.rows('.selected').data()[0];
	
	// Disable delete button after record has been deleted.
	saveEntry(postUrl, row, null, "The Definition has been deleted.", tbl,
			function(){	
				tbl.rows().deselect();
				updateDefinitionToolbarButtons();
			}
	...
	var buttonsArray = [
		{
			...
			className: "btn btn-sm",
			action: function( e, dt, node, config ) {
				editTaskType(null, row.definitionId);
			}
		},
		{
			attr: {
...
//	console.log("/rest/ignite/v1/task-type/by-parent/" + row.definitionId);
	
	taskTypeTable = initializeGenericTable("taskTypeTable", 
			                            "/rest/ignite/v1/task-type/by-parent/" + row.definitionId,
			                            columnArray,
			                            columnDefsArray,
			                            buttonsArray,
			                            function(rowSelector) {
											editTaskType(rowSelector, row.definitionId);
										},
										null,
										25,
...
function editTaskType(rowSelector, currentDefinitionId) {
	...
	$("#ttDlgDefinitionId").val(currentDefinitionId);
...
// Maak hierdie true	
	...
	$("#ttDlgDefinitionId").prop("readonly", true);
...
//    console.log('dieMinute: ' + dieMinute)
	...
	var postData = {
			...
			definitionId: $("#ttDlgDefinitionId").val(),
